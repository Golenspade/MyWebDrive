...
test('browser documentation requires explicit Linux snapshot updates', async () => {
  ...
  assert.match(docs, /SMOKE_ALLOW_HOST_SNAPSHOTS/)
  assert.match(docs, /temporary/)
  assert.match(docs, /When a Linux development machine exists/)
})
...
test('snapshot update documentation requires verified Linux container provenance', async () => {
  ...
  assert.match(mode, /SMOKE_ALLOW_HOST_SNAPSHOTS must be 0 or 1/)
})
