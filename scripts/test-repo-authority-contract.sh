...
if [[ -f "$GIT_WORKFLOW_DOC" ]]; then
  ...
  require_text "$GIT_WORKFLOW_DOC" '## Protected history sync' 'Git workflow protected history-sync section'
  require_text "$GIT_WORKFLOW_DOC" 'Before opening `develop -> main`, run the protected history sync if `main` is not already an ancestor of `develop`.' 'Git workflow pre-release history-sync gate'
  require_text "$GIT_WORKFLOW_DOC" 'Create a short-lived sync branch from the latest `develop`.' 'Git workflow protected sync branch'
  require_text "$GIT_WORKFLOW_DOC" 'Merge the current `main` into the sync branch with a normal merge commit.' 'Git workflow protected sync merge'
  require_text "$GIT_WORKFLOW_DOC" 'Open a Pull Request from the sync branch to `develop`.' 'Git workflow protected sync Pull Request'
  require_text "$GIT_WORKFLOW_DOC" 'Use Merge commit, not Squash merge, so the `main` history remains reachable from `develop`.' 'Git workflow protected sync merge method'
  require_text "$GIT_WORKFLOW_DOC" 'Wait for the exact resulting `develop` push to succeed and verify `Publish immutable images` is `skipped`.' 'Git workflow protected sync publish boundary'
