...
function verifyShareAccessToken(token) {
    ...
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        if (decoded?.type === 'share_access' && decoded?.share_id)
...
app.post('/api/v1/shares/:shareToken/access', async (req, res, next) => {
    try {
        ...
        if (share.expiresAt && share.expiresAt.getTime() < Date.now())
            return res.status(410).json({ error: 'Share expired' });
        if (share.lockUntil && share.lockUntil.getTime() > Date.now())
...
app.get('/api/v1/shares/:shareToken/download', async (req, res, next) => {
    try {
        const token = req.params.shareToken;
        const access = req.headers['x-share-access-token'] || req.query['access_token'] || '';
        const share = await prisma.share.findUnique({ where: { token } });
        if (!share || !share.isActive)
            return res.status(404).json({ error: 'Share not found' });
        if (share.expiresAt && share.expiresAt.getTime() < Date.now())
        ...
        if (share.passwordHash) {
            ...
            if (shareId !== share.id)
                return res.status(401).json({ error: 'Share access required' });
        }
...
app.patch('/api/v1/shares/:shareId', requireAuth, async (req, res, next) => {
    try {
        ...
        const ownerId = req.auth.userId;
        const { password, expiresAt, isActive } = (req.body || {});
...
app.post('/api/v1/shares/:shareId/permissions', requireAuth, async (req, res, next) => {
    try {
        const shareId = req.params.shareId;
        const ownerId = req.auth.userId;
        const { maxDownloads, expiresAt, password } = (req.body || {});
        const share = await prisma.share.findUnique({ where: { id: shareId } });
        if (!share || share.ownerId !== ownerId)
            return res.status(404).json({ error: 'Share not found' });
        const data = {};
        if (typeof maxDownloads === 'number' || maxDownloads === null)
...
function checkRateLimit(ip, shareId) {
