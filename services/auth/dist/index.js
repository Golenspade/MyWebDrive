...
try {
    ...
    logger.info({ proto: u.protocol, host: u.host, pathname: u.pathname }, 'auth db url');
}
catch { }
    ...
    try {
        ...
        if (REQUIRE_INVITE) {
            ...
            if (inv.expiresAt && inv.expiresAt.getTime() < Date.now())
                return res.status(403).json({ error: 'Invalid or expired invitation code' });
            if (inv.usedCount >= inv.usageLimit)
                return res.status(403).json({ error: 'Invalid or expired invitation code' });
        }
...
app.post('/api/v1/auth/owner-login', async (req, res) => {
    ...
    try {
        ...
        if (!code)
            return res.status(400).json({ error: 'Invalid request' });
        if (!OWNER_CODE_HASH)
            return res.status(503).json({ error: 'Owner code not configured' });
...
app.post('/api/v1/auth/refresh', async (req, res, next) => {
    try {
        ...
        if (decoded?.type !== 'refresh' || !decoded?.user_id)
...
app.post('/api/v1/auth/invitations', requireAuth, requireAdmin, async (req, res, next) => {
    try {
        const { usageLimit, expiresAt, notes } = (req.body || {});
